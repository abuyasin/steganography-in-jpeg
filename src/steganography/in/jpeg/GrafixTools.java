 ...
 * <br><br>
 * This is a collection of graphics mundanes that I had to cut and paste one too
 * many times. While laziness on my part I think that I was doing this stuff
 * enough to warrant packaging up these goodies into their own proper class.
 * Hopefully this will evolve into something that's worth releasing.
 * <br><br>
 * <b>Feature list:</b><br>
...
public class GrafixTools {
    ...
    /**
     * Gives GraphixTools a preloaded image object. Image must be pre-loaded
     * with the MediaTracker or bad things will happen. The constructor
     * calculates all the image parameters from this passed image. (Dimensions,
     * size, etc.)
     *
    ...
    /**
     * This method returns an array[][] of 8-bit green values. The dimensions
     * are the array bounds ex. (320 x 200).
     *
     ...
     * technically correct when referred to a YCbCr colorspace.) YCbCr was
     * developed according to recommondation ITU-R BT.601 (Formerly: CCIR 601)
     * to develop a world-wide digital video standard. It is commonly used in
     * many streaming video and compression routines, such as
     * <a href="http://www.mpeg.org">MPEG</a>
     * and <a href="http://www.telenor.nl">H.263.</a>
     * <br><br>
     * This routine handles dealing with 0..255 values per pixel, the more
     * common PC format. The passed arrays should all have the same bounds,
     ...
     * array of 32 bit integers. This method can take a long time to complete so
     * a reporting feature has been added. The arrays should be the same
     * dimensions as imageWidth and imageHeight. This array can then be
     * converted to an image with: <br>
     * newimage = comp.createImage(new
     * MemoryImageSource(imageWidth,imageHeight,imageArray,0,imageWidth));
     *
     ...
     * time to complete so a reporting feature has been added. The arrays should
     * be the same dimensions as imageWidth and imageHeight. This array can then
     * be converted to an image with: <br>
     * newimage = comp.createImage(new
     * MemoryImageSource(imageWidth,imageHeight,imageArray,0,imageWidth));
     *
